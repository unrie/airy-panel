...
<?php echo $header ?>
				...
				<div class="btn-group">
					<a href="/admin/servers/index?locationid=<?php echo $location['location_id'] ?>" class="btn btn-default"><span class="glyphicon glyphicon-hdd"></span> Серверы локации</a>
				...
				<form class="form-horizontal" action="#" id="editForm" method="POST">
					...
					<div class="form-group">
						<div class="col-sm-offset-3 col-sm-9">
							<div class="checkbox">
								<label><input type="checkbox" id="editdbpassword" name="editdbpassword" onChange="toggleDbPassword()"> Изменить пароль БД</label>
							</div>
						</div>
					</div>
					<div class="form-group">
						<label for="dbpassword" class="col-sm-3 control-label">Пароль БД:</label>
						<div class="col-sm-4">
							<input type="password" class="form-control" id="dbpassword" name="dbpassword" placeholder="Пароль БД" disabled>
						</div>
					</div>
					<div class="form-group">
						<label for="dbpassword2" class="col-sm-3 control-label">Повторите пароль БД:</label>
						<div class="col-sm-4">
							<input type="password" class="form-control" id="dbpassword2" name="dbpassword2" placeholder="Повторите пароль БД" disabled>
						</div>
					</div>
				...
				<script>
					...
					function toggleDbPassword() {
						var status = $('#editdbpassword').is(':checked');
						if(status) {
							$('#dbpassword').prop('disabled', false);
							$('#dbpassword2').prop('disabled', false);
						} else {
							$('#dbpassword').prop('disabled', true);
							$('#dbpassword2').prop('disabled', true);
						}
					}
				</script>
<?php echo $footer ?>
