				...
				<div class="panel panel-<?php if($server['server_status'] == 0){echo 'warning';}elseif($server['server_status'] == 1){echo 'danger';}elseif($server['server_status'] == 2){echo 'success';}elseif($server['server_status'] == 3){echo 'warning';}?>">
					<div class="panel-heading">Информация о сервере</div>
						...
						<tr>
							...
							<td><button type="button" class="btn btn-primary btn-xs" onClick="redirect('/servers/slots/index/<?php echo $server['server_id'] ?>')"><span class="glyphicon glyphicon-edit"></span> Изменить</button></td>
						</tr>
						<tr>
							<th>Дата окончания оплаты:</th>
							<td><?php echo date("d.m.Y", strtotime($server['server_date_end'])) ?></td>
							<td><button type="button" class="btn btn-primary btn-xs" onClick="redirect('/servers/pay/index/<?php echo $server['server_id'] ?>')"><span class="glyphicon glyphicon-usd"></span> Оплатить</button></td>
						</tr>
						<tr>
							<th>Статус:</th>
							<td>
