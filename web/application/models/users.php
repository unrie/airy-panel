...
class usersModel extends Model {
	public function createUser($data) {
		...
		$sql .= "user_mailing = '" . (int)$data['user_mailing'] . "', ";
		$sql .= "user_theme = '" . (int)$data['user_theme'] . "', ";
	...
	public function getUserByRestoreKey($userrestorekey) {
		$sql = "SELECT * FROM `users` WHERE `user_restore_key` = '" . $this->db->escape($userrestorekey) . "' LIMIT 1";
		$query = $this->db->query($sql);
		return $query->row;
	}
