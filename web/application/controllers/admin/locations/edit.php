...
class editController extends Controller {
	...
	public function ajax($locationid = null) {
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST') {
			...
			if(!$errorPOST) {
				...
				$editdbpassword = @$this->request->post['editdbpassword'];
				$dbpassword = @$this->request->post['dbpassword'];
				$dbpassword2 = @$this->request->post['dbpassword2'];
				...
				if($editdbpassword) {
					$locationData['location_db_password'] = $dbpassword;
				}
	...
	private function validatePOST() {
		...
		elseif(mb_strlen(trim($user)) < 2 || mb_strlen(trim($user)) > 32) {
		...
		elseif($editdbpassword) {
			if(!$validateLib->password($dbpassword)) {
				$result = "Пароль БД должен содержать от 6 до 32 латинских букв, цифр и знаков <i>,.!?_-</i>!";
			}
			elseif($dbpassword != $dbpassword2) {
				$result = "Введенные вами БД пароли не совпадают!";
			}
		}
