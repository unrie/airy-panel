...
def serverInstall():
	...
	os.system('echo "create database ' + username + '" | mysql -uroot -p' + dbpassword)
	os.system('echo "grant usage on *.* to ' + username + '@\'%\' identified by \'' + password + '\'" | mysql -uroot -p' + dbpassword)
	os.system('echo "grant all privileges on ' + username + '.* to ' + username + '@\'%\'" | mysql -uroot -p' + dbpassword)
	return True
...
def serverStart():
	...
	os.system('rm -f /home/' + username + '/screenlog.0')
	os.system('su -lc "screen -AmdSL gameserver ' + execCmd + '" ' + username)
	return True
...
def serverBackup():
  ...
  filename = date.strftime('%d-%m-%y-%H-%M-%S') + '.tar.gz'
  os.system('mysqldump -u ' + username + ' -p' + password + ' ' + username + ' > /home/' + username + '/' + username + '.sql')
  os.system('tar -czf /home/' + username + '/backups/' + filename + ' --exclude="backups" -C / home/' + username + '/')
  return True

def serverUpdatePassword():
	os.system('usermod -p ' + crypt.crypt(password, salt) + ' ' + username)
	os.system('echo "UPDATE mysql.user SET Password=PASSWORD(\'' + password + '\') WHERE User=\'' + username + '\'" | mysql -uroot -p' + dbpassword)
	os.system('echo "FLUSH PRIVILEGES;" | mysql -uroot -p' + dbpassword)
	return True

def serverDelete():
	os.system('userdel -rf ' + username)
	os.system('echo "drop user ' + username + '" | mysql -uroot -p' + dbpassword)
	os.system('echo "drop database ' + username + '" | mysql -uroot -p' + dbpassword)
	return True
...
if action == 'backup':
  ...
  if not serverBackup():
    returnResult('ERROR', 'BackupError')
  else:
    returnResult('OK', '')
